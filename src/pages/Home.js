      ...
      <div className="min-h-full">
        ...
        <main>
          ...
          <div className="flex mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 items-center">
            <h1 className="text-3xl font-medium tracking-tight text-gray-700 mr-2">Daftar Bunga </h1><QueueListIcon className="h-8 w-8" aria-hidden="true" />
          </div>
          <div className='grid grid-cols-3 gap-4 place-items-stretch'>
            {data.todo.map((item) => (
              <div class="flex m-3 h-20 basis-1/4 bg-orange-300 rounded-lg items-center place-content-center">
                <span className='font-bold text-3xl text-gray-900'>{item.task}</span>
              </div>
            ))}
          </div>
        </main>
